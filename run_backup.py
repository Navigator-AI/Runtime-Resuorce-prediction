...
def train(req: TrainRequest):
    try:
        ...
        pred_rt = yscaler_rt.inverse_transform(rt_model.predict(X, verbose=0)).reshape(-1)
        pred_mb = yscaler_mb.inverse_transform(mb_model.predict(X, verbose=0)).reshape(-1)
        ...
        mult_rt = max(1.0, float(np.percentile(ratio_rt_valid, 90))) if len(ratio_rt_valid)>0 else 1.0
        mult_mb = max(1.0, float(np.percentile(ratio_mb_valid, 90))) if len(ratio_mb_valid)>0 else 1.0
        ...
        return {"message": f"✅ Model trained for {req.table}", "table": req.table,
                "r2_rt": float(r2_rt), "r2_mb": float(r2_mb),
                "calib_rt": mult_rt, "calib_mb": mult_mb}
...
def predict(req: PredictRequest):
    try:
        ...
        if req.block and block_col in df.columns:
            df = df[df[block_col].astype(str).str.lower() == req.block.lower()]
        if req.stage:
            df = df[df[stage_col].astype(str).str.lower() == req.stage.lower()]
        if df.empty:
            raise HTTPException(status_code=404, detail="No rows match given block/stage filters")
        ...
        pred_rt *= float(meta.get("calib_rt", 1.0))
        pred_mb *= float(meta.get("calib_mb", 1.0))
        ...
        for i in range(len(pred_rt)):
            ...
            out.append({"block": block_val, "stage": stage_val,
                        "predicted_runtime": prt, "predicted_memory": pmb})
        ...
        return {"message": "✅ Prediction successful", "table": req.table,
                "block": req.block, "stage": req.stage,
                "rows": int(results.shape[0]), "file": fname,
                "preview": results.to_dict(orient="records")}
